...
import { deleteFaqAction, togglePublishFaqAction } from '@/lib/actions/faq';
...
export default async function FaqPage() {
  ...
  const { data } = await supabase
    .from('faq')
    .select('id, question, sort, is_published')
    .order('sort');
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-[28px] font-extrabold uppercase tracking-tight2">
          FAQ <span className="text-ink-2 text-[18px]">({rows.length})</span>
        </h1>
            ...
            <tr key={r.id}>
              ...
              <Td>
                <PublishToggle
                  ...
                  onToggle={async (next) => {
                    'use server';
                    await togglePublishFaqAction(r.id, next);
                  }}
                />
              </Td>
              <Td className="text-right">
                <Link href={`/admin/faq/${r.id}`}>
                  <Button variant="outline" size="sm">Редагувати</Button>
                </Link>{' '}
                <DeleteButton
                  onDelete={async () => {
                    'use server';
                    await deleteFaqAction(r.id);
                  }}
                />
              </Td>
            </tr>
